...
class WARP(Function):
    @staticmethod
    def forward(ctx, X, y, max_num_trials=None):
        
        batch_size = y.size()[0]
        if max_num_trials is None:
            max_num_trials = y.size()[1] - 1
        
        positive_indices = torch.zeros(X.size()).cuda()
        negative_indices = torch.zeros(X.size()).cuda()
        L = torch.zeros(X.size()[0]).cuda()
        
        all_labels_idx = np.arange(y.size()[1])
        ...
        for i in range(batch_size): 
            ...
            msk = np.ones(y.size()[1], dtype=bool)
            ...
            while ((sample_score_margin < 0) and (num_trials < max_num_trials)):
                ...
                neg_idx = np.random.choice(neg_labels_idx, 1)
                msk[neg_idx] = False
                neg_labels_idx = all_labels_idx[msk]
                
                num_trials += 1
                # calculate the score margin 
                sample_score_margin = 1 + X[i, neg_idx] - X[i, j]
        ...
        loss = L * (1-torch.sum(positive_indices*X, dim=1) + torch.sum(negative_indices*X, dim=1))
...
class WARPLoss(nn.Module):
    ...
    def forward(self, X, y):
        return WARP.apply(X, y, self.max_num_trials)